            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <div className="bg-slate-900 px-5 py-4 flex items-center justify-between text-white">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-indigo-500 rounded-full flex items-center justify-center shadow-inner relative">
                                    <Bot size={22} />
                                    <div className={`absolute -bottom-1 -right-1 w-3 h-3 border-2 border-slate-900 rounded-full ${isSpeaking ? 'bg-green-500' : 'bg-slate-500'}`}></div>
                                </div>
                                <div>
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setUseSpeech(!useSpeech)} className={`p-2 rounded-lg transition-colors ${useSpeech ? 'text-indigo-400 bg-white/5' : 'text-slate-500 hover:text-white'}`}>
                        ...
                        <div className="h-[380px] overflow-y-auto p-5 bg-slate-50/50 flex flex-col gap-4">
                            {mode === 'menu' ? (
                                <div className="h-full flex flex-col items-center justify-center text-center p-4">
                                    ...
                                    <h5 className="font-bold text-slate-900 mb-2">How can I help you?</h5>
                                    ...
                                    <div className="w-full flex flex-col gap-3">
                                        ...
                                        <button
                                            ...
                                            className="flex items-center justify-center gap-3 bg-white text-slate-700 border border-slate-200 py-4 rounded-xl font-bold hover:border-indigo-300 transition-all"
                                        >
                                ...
                                <>
                                    {messages.length === 0 && mode === 'chat' && (
                                        <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-xl text-center">
                                            <p className="text-xs text-indigo-700 font-medium">I'm ready! Ask about our work, tech, or pricing.</p>
                                        </div>
                                    )}
                                    {messages.map((m) => (
                                        <motion.div key={m.id} initial={{ opacity: 0, x: m.sender === 'user' ? 20 : -20 }} animate={{ opacity: 1, x: 0 }} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${m.sender === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white shadow-sm border border-slate-100 text-slate-700 rounded-tl-none'}`}>
                                                {m.text}
                                            </div>
                                        </motion.div>
                                    ))}
                        ...
                        <form onSubmit={handleSendMessage} className={`p-4 bg-white border-t border-slate-100 flex items-center gap-2 ${mode !== 'chat' ? 'opacity-30 pointer-events-none' : ''}`}>
                            <button type="button" className="p-2 text-slate-400 hover:bg-slate-50 rounded-full"><Mic size={20} /></button>
                            <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Type a message..." className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm focus:outline-none" />
